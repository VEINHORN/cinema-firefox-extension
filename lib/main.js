...
var panel = panels.Panel({
  ...
  contentScriptFile: [ self.data.url("js/libs/jquery-2.1.3.min.js"), self.data.url("js/popup.js") ],
...
panel.port.on("resize-popup-width", function(width) {
  panel.width = width;
  console.log("Popup width resized.");
});
...
function handleChange(state) {
  if(state.checked) {
    ...
    panel.port.emit("focus-search-field");
    console.log("Focused on a search field.");
  }
}
